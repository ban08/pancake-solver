...
class PancakeGUI:
    """Simple Tkinter interface for loading, playing, and solving pancake puzzles."""
    ...
    def _build_ui(self) -> None:
        ...
        algorithm_combo = ttk.Combobox(
            ...
            state="readonly",
            values=["BFS", "DFS", "IDS", "UCS", "Greedy", "A*", "Weighted A*"],
            width=14,
        )
        ...
        # Left side: visual stack + manual controls
        ...
        self.canvas.bind("<Configure>", lambda _event: self.draw_state())

        self.flip_buttons_frame = ttk.LabelFrame(
            left_frame,
            text="Manual Flip Controls",
            padding=10,
        )
        self.flip_buttons_frame.grid(row=1, column=0, sticky="ew", pady=(10, 0))

        # Right side: stats + output box
        ...
        ttk.Label(info_frame, textvariable=self.moves_var).grid(
    ...
    def draw_state(self) -> None:
        ...
        n = len(stack)
        pancake_height = max(28, min(60, (canvas_height - 90) // max(1, n)))
        top_margin = 45
        center_x = canvas_width // 2
        max_width = min(520, canvas_width - 80)
        min_width = 100
        max_value = max(stack)
    ...
    def show_hint(self) -> None:
        ...
        moves = getattr(result, "solution_moves", []) or []
    ...
    def play_solution(self) -> None:
        ...
        self._cancel_playback()
        self.current_state = self.solution_states[0]
        self.playback_index = 0
    ...
    def _play_next_state(self) -> None:
        ...
        if self.playback_index < len(self.solution_states):
            self.playback_job = self.root.after(500, self._play_next_state)
        else:
            self.playback_job = None
    ...
    def _store_solution(self, result) -> None:
        self.solution_states = list(getattr(result, "solution_states", []) or [])
        self.solution_moves = list(getattr(result, "solution_moves", []) or [])

    def _show_solution_summary(self, result) -> None:
        solved = getattr(result, "solved", False)
        moves = getattr(result, "solution_moves", []) or []
        states = getattr(result, "solution_states", []) or []

        lines = [
            f"Algorithm: {self.algorithm_var.get()}",
            (
                f"Heuristic: {self.heuristic_var.get()}"
                if self.algorithm_var.get() in {"Greedy", "A*", "Weighted A*"}
                else "Heuristic: None"
            ),
        ]
        ...
        lines.extend(
            [
                f"Solved: {solved}",
                f"Solution cost: {getattr(result, 'solution_cost', '-')}",
                f"Moves: {moves}",
                "",
            ]
        )
    ...
    def _on_algorithm_change(self, event=None) -> None:
        ...
        self.heuristic_combo.configure(state="readonly" if uses_heuristic else "disabled")
        self.weight_entry.configure(state="normal" if uses_weight else "disabled")
    ...
    @staticmethod
    def _extract_stack(state: PancakeState | list[int] | tuple[int, ...]) -> list[int]:
